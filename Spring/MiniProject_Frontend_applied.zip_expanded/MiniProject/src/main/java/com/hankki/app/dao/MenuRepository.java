...
public class MenuRepository implements IMenuRepository {
	...
	@Override
	public List<MenuVO> getStoreMenulist() {
	...
	public MenuVO getMenuInfo(int menuid) {
		String sql = "select id, store_id, name, cost,"
				+ "img_str, star, comments"
				+ "from menu where id=?";		
	...
	public void updateMenu(MenuVO menu) {
		String sql = "update menu "
				+ "set store_id=?, name=?, cost=?,"
				+ "img_str=?, star=?, comments=?"
				+ "where id=?";
		jdbcTemplate.update(sql, menu.getStoreId(),
				menu.getMenuName(),
				menu.getCost(),
				menu.getImgStr(),
				menu.getMenuId()
